...
function getServiceUnitFiles(service, unitNameBase, instance) {
  ...
  if (service.config.volume) {
    Object.keys(service.config.volume).forEach(function(key) {
      iniObj.Service.ExecStart += ' -v ' + key + ' ' + service.config.volume[key];
    });
  }
  iniObj.Service.ExecStart += ' ' + service.dockerRepository;
  iniObj.Service.ExecStop = '/usr/bin/docker rm -f ' + unitName;
